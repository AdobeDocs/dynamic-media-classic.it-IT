...
1. Nell’area Mappa immagine, disegnate una mappa immagine rettangolare o poligonale:
   ...
   * **Mappa rettangolare** - Seleziona lo strumento Mappa immagine rettangolo e trascina sulla pagina per creare il rettangolo.

   * **Mappa poligonale** - Selezionate lo strumento Mappa immagine poligonale, quindi selezionate il numero di volte necessario intorno al perimetro dell&#39;immagine. Quando selezionate, Adobe Dynamic Media Classic disegna i bordi della mappa immagine.

      Dopo aver disegnato una mappa immagine, Adobe Dynamic Media Classic le assegna un nome nell’elenco Mappa immagine. Per creare il nome, Adobe Dynamic Media Classic aggiunge un numero sequenziale al nome della pagina eCatalog in cui si sta lavorando.

1. (Facoltativo) Dall’elenco Mappa immagine, nella [!UICONTROL Nome] È possibile immettere un nuovo nome per la mappa immagine. Non includete spazi nel nome inserito.
1. È possibile fare in modo che i visualizzatori aprano una nuova pagina web quando selezionano la mappa immagine. Nell’elenco delle mappe immagini, inserite l’URL della pagina Web nella colonna URL.
   ...
   Per semplificare l’immissione degli URL (modelli Href), seleziona **[!UICONTROL Modifica]** e immetti un modello.
...
1. (Facoltativo) Nell’elenco a discesa Mostra , seleziona **[!UICONTROL Testo di rollover]**, quindi immetti il testo che gli utenti dovranno visualizzare sullo schermo quando passano il puntatore del mouse sulla mappa immagine.
...
1. (Facoltativo) Seleziona **[!UICONTROL Anteprima]** per visualizzare l’eCatalog con il predefinito visualizzatore di eCatalog predefinito.
...
1. Disegnate una mappa immagine nell’eCatalog.
   ...
   Vedi [Disegnare mappe immagine di eCatalog](creating-ecatalog-image-maps.md#drawing_ecatalog_image_maps).

1. Nell’elenco a discesa Mostra , seleziona **[!UICONTROL Contenuti multimediali]**.
...
1. (Facoltativo) Nel pannello elenco Mappa immagine, sotto la sezione **[!UICONTROL Descrizione comandi]** nell’intestazione della colonna, immetti il testo che gli utenti dovranno visualizzare sullo schermo quando posizionano il puntatore del mouse sulla mappa immagine.
1. Seleziona **[!UICONTROL Salva]**.
...
* **Regolare la posizione** - Selezionare lo strumento Panning e spostare il puntatore vicino ma non oltre il bordo della mappa. Quando il puntatore diventa una freccia a quattro punte, trascinate l’intera mappa immagine in una nuova posizione.

   Vedi [Regolare la posizione, la forma e le dimensioni delle mappe immagine](creating-image-maps.md#adjusting_the_position_shape_and_size_of_image_maps).
...
* **Eliminazione delle mappe immagine** - Selezionare lo strumento Panning, selezionare la mappa immagine per selezionarla, quindi selezionare **[!UICONTROL Elimina]**.

   Per rimuovere tutte le mappe immagine da un eCatalog, seleziona la **[!UICONTROL Pagine ordine]** , quindi seleziona **[!UICONTROL Cancella mappe]**.
...
* **Gestione delle mappe immagine sovrapposte** - Trascinare per modificare l’ordine delle mappe immagine nell’elenco Mappa immagine.
   ...
   Vedi [Gestire le mappe immagine sovrapposte](creating-image-maps.md#handling_overlapping_image_maps).

* **Copia delle mappe immagine in altre pagine** - Seleziona **[!UICONTROL Copia mappe in]** Assicurati di essere nella scheda Mappa pagine . Nella schermata Seleziona immagini , seleziona la pagina o le pagine in cui desideri copiare le mappe immagine e seleziona **[!UICONTROL Seleziona]**.

   Vedi [Copiare le mappe immagine in altre immagini](creating-image-maps.md#copying_image_maps).

>[!NOTE]
>
>Oltre a copiare le mappe immagine in pagine diverse di un eCatalog, puoi copiare tutte le mappe immagine di un eCatalog in un altro eCatalog. Vedi [Copiare mappe immagine tra gli eCatalogs](creating-ecatalog-image-maps.md#copying_image_maps_between_ecatalogs).
...
1. In caso di errori del tasto di rollover, seleziona l’errore nel **[!UICONTROL Errore Rollover_Key]** per vedere cosa deve cambiare nel foglio di calcolo per correggere l’errore. Potete selezionare e copiare il testo di questo messaggio e incollarlo nel foglio dati.
1. Seleziona **[!UICONTROL Anteprima]** potete esaminare una pagina nel visualizzatore di eCatalog; seleziona la X per chiudere la schermata Riepilogo e tornare alla schermata Mappa pagine oppure seleziona **[!UICONTROL Chiudi]** per tornare a Sfoglia.
...
1. Nella finestra di dialogo Importa metadati, seleziona **[!UICONTROL Sfoglia]**, quindi selezionare il file DTD XML o delimitato da tabulazioni.
...
1. Selezionate l’eCatalog con le mappe immagine da copiare e selezionate il rollover dell’eCatalog **[!UICONTROL Modifica]** pulsante .
...
Adobe Dynamic Media Classic visualizza un messaggio di avviso se l’eCatalog di destinazione (l’eCatalog in cui copiate le mappe immagine) ha un numero diverso di pagine o immagini di dimensioni diverse. Seleziona **[!UICONTROL Continua]** per copiare le mappe immagine nonostante l’avviso.
