...
description: Scopri come creare un modello in Adobe Systems Dynamic Media Classic.
...
Per creare un modello, vai a **[!UICONTROL Build]** > **[!UICONTROL template Basics]** . Selezionate Designer o Sviluppatore. In questa pagina potete aggiungere livelli di immagine e testo. Potete anche riordinare livelli, modificarne le dimensioni e la posizione e applicare effetti di ombra e bagliore alle immagini e al testo.
...
Se necessario, potete creare un modello partendo da uno già esistente. Aprite il modello, selezionate **[!UICONTROL Salva As]** e immettete un nuovo nome nella finestra di dialogo Salva As.
...
1. Seleziona una cartella per memorizzare il modello, immetti un nome per il modello e seleziona **[!UICONTROL Invia]** .
...
1. Seleziona **[!UICONTROL Salva]** , seleziona una cartella di archiviazione, immetti un nome per il gruppo, quindi seleziona **[!UICONTROL Salva]** .

## Elimina un modello {#deleting-a-template}
...
* **[!UICONTROL Il riepilogo]** dei parametri apre la schermata Riepilogo parametri. Potete visualizzare il nome di ciascun livello in un modello e per ciascun livello i nomi dei parametri che sono stati attivati.

* **[!UICONTROL Editor di testo v 4.3 e editor di testo v 4.2]** -puoi scegliere di utilizzare il testo più recente e più editor, l&#39;editor di testo v 4.3 o il editor di testo precedente, l&#39;editor di testo v 4.2. Durante la creazione dei modelli, l&#39;editor di testo v 4.3 è selezionato per impostazione predefinita. Per la modifica di modelli meno recenti, per impostazione predefinita viene selezionato Editor di testo v4.2. Editor di testo v4.3 attualmente non supporta il ritorno a capo automatico, quindi per modificare i modelli meno recenti con la funzione di ritorno a capo automatico, usate Editor di testo v4.2. Se il modello meno recente non utilizza il ritorno a capo automatico, è possibile scegliere Editor di testo v. 4.3 per sfruttare le numerose nuove funzionalità offerte. Ad esempio, aumentare i margini, diminuire i margini, impostare il testo in maiuscolo e copiare il testo adattato.
...
* **[!UICONTROL Designer e sviluppatore]** -Seleziona l&#39;opzione che descrive al meglio il tuo ruolo.
...
* **[!UICONTROL Area]** Proprietà livello-offre strumenti per modificare il colore di sfondo, l&#39;opacità, le dimensioni e la posizione di un livello, così come il colore di sfondo, l&#39;opacità e le dimensioni della tela. Potete anche regolare gli effetti di ombra e bagliore. Consultate [ lavorare con i livelli ](#working-with-layers) .
...
1. Nella casella di testo dell&#39;editor di testo, selezionate il testo che desiderate formattare. Potete selezionare tutto il testo, porzioni di testo o singoli caratteri.
1. Specifica una delle seguenti opzioni di formattazione, quindi seleziona **[!UICONTROL Applica]** .

   * **[!UICONTROL Font]** -scegliete un font dal menu Font. Se un font desiderato non compare nel menu, potete caricarlo nella Adobe Systems Dynamic Media Classic. Consultate Font.
...
1. Nell’elenco Livelli, fate doppio clic sul nome della casella di testo contenente il testo da modificare. Viene aperto l’editor di testo.
1. Seleziona il paragrafo da formattare.
1. Specifica una delle seguenti opzioni di formattazione, quindi seleziona **[!UICONTROL Applica]** .

   * **[!UICONTROL Allineamento]** -selezionare questa opzione per specificare il tipo di allineamento: Allinea a sinistra, Allinea al centro, Allinea a destra o giustifica.
   ...
   * **[!UICONTROL Rientro First line]** -specifica la quantità di cui desideri far rientrare la prima riga di testo.

   * **[!UICONTROL Spazio prima del paragrafo]** -specificate la quantità di spazio che desiderate visualizzare sopra la prima riga di testo del paragrafo.
...
1. Nel pannello Proprietà livello, effettuate una delle seguenti operazioni:
   ...
   * **[!UICONTROL Riduci testo (solo editor di testo v 4.2)]** -per adattarlo all&#39;interno della casella di testo, selezionare per ridurre il testo.

   * **[!UICONTROL Word wrap (solo editor di testo v 4.2)]** -per specificare se o come il testo si avvolge, seleziona un&#39;opzione di avvolgimento:
...
## Lavorare con i livelli {#working-with-layers}
...
1. Per riordinare i livelli in un modello, effettuate una delle seguenti operazioni:
   ...
   * Selezionate un livello nel pannello Livelli. Quindi seleziona **[!UICONTROL up]** o **[!UICONTROL down]** quante volte necessario per posizionarlo nella posizione corretta nell&#39;elenco.
   * Trascinate un livello verso l’alto o il basso nell’elenco Livelli.

### Modificare la dimensione e la posizione dei livelli e dell&#39;area di lavoro {#changing-the-size-and-position-of-layers-and-the-canvas}
...
>[!NOTE]
>
>Adobe Systems Dynamic Media Classic consiglia di creare un predefinito di Immagine che corrisponde alla dimensione esatta del modello. Quando le dimensioni del predefinito per immagini corrispondono a quelle del modello, si otterranno impostazioni corrette per le dimensioni di output finali e le opzioni di nitidezza per il modello. Dopo aver creato il predefinito per immagini, potete sceglierlo dal menu Applica predefinito sulla schermata Anteprima modello. Nella schermata viene visualizzato l’aspetto dell’immagine che verrà trasmessa dal server. Consultate [ configurare Immagine predefiniti ](setting-image-presets.md#setting_up_image_presets) .
...
* **Rotazione** manuale: sposta il cursore vicino ma non sopra un angolo del livello. Quando compare il cursore di rotazione, trascinate l’angolo del livello. Per ruotare per incrementi di 15 gradi, tenete premuto il tasto Maiusc mentre trascinate.

* **Immissione di una misura** di grado-immettete il numero di gradi per la rotazione del livello. La rotazione è in senso orario; per ruotare in senso antiorario, immettete un numero negativo.
...
### Determinare il colore di sfondo, l&#39;opacità e la modalità di fusione {#determining-the-background-color-opacity-and-blend-mode}
...
* **Sfondo colore** , selezionare **[!UICONTROL colore di sfondo]** e scegliere un colore campione per modificare il colore delle aree imbottite.

* **Opacità** -trascinate la Opacità cursore per rendere qualsiasi livello traslucido in modo che la parte dell&#39;immagine sottostante venga visualizzata. L&#39;impostazione 100% è opaca; 0 è trasparente.

* **Modalità** fusione-per simulare uno dei modi di fusione disponibili in Photoshop, scegliete un&#39;opzione. Le opzioni sono Normale, Dissolvenza, Schiarisci, Scurisci, Moltiplica e Scolora. Queste opzioni sono disponibili per i livelli, ma non per il quadro.

## Utilizzare gli effetti ombra e bagliore sui livelli {#using-shadow-and-glow-effects-on-layers}

Potete applicare un’ombra o un bagliore a un livello. L’ombra o il bagliore viene applicato al perimetro del livello e si estende verso l’interno o l’esterno, a seconda dell’opzione scelta. Se il modello è stato creato con un file PSD con effetti ombra e bagliore, potete regolare questi effetti in Adobe Systems Dynamic Media Classic.
...
### Applica un effetto ombra o bagliore su un livello {#applying-a-shadow-or-glow-effect-to-a-layer}
...
* **[!UICONTROL Colore]** -selezionate il Colore pulsante e scegliete un campione colore per modificare il colore dell&#39;ombra o del bagliore. Potete anche immettere un parametro con il valore del colore nella casella.
...
* **[!UICONTROL Metodo fusione]** -per simulare uno dei modi di fusione disponibili in Photoshop, scegliete un&#39;opzione. Le opzioni sono Normale, Dissolvenza, Schiarisci, Scurisci, Moltiplica e Scolora.

* **[!UICONTROL Dimensione]** -immettete le misurazioni nella casella X e Y per ingrandire o ridurre l&#39;effetto ombra. Le opzioni Dimensione sono disponibili per le ombre interne ed esterne.

* **** Trascinate il cursore per estendere l&#39;effetto verso l&#39;interno o verso l&#39;esterno.
