---
title: Strumentazione di un visualizzatore con Adobe Analytics Instrumentation Kit
...
Dove `Adobe Dynamic Media Classic Company ID` è impostato sul nome della società Adobe Dynamic Media Classic. E `&preset` è facoltativo. Se il nome del predefinito della società non è `companypreset`, non è facoltativo. In questi casi si tratta di `companypreset-1`, `companypreset-2` e versioni successive. Le istanze più recenti del predefinito hanno un numero maggiore. Per determinare il nome del predefinito società corretto, selezionare **[!UICONTROL Copia URL]**, quindi controllare il parametro `preset=` per trovare il nome del predefinito società.

Aggiungi una funzione che trasmette l’evento visualizzatore al codice di tracciamento di Adobe Analytics.

Aggiungere la funzione `s7ComponentEvent()` al HTML contenitore (o JSP, ASPX o altro):

```as3
function s7ComponentEvent(objectId, componentClass, instanceName, timeStamp, eventData) {     s7track(eventData); }
```

Il nome della funzione distingue tra maiuscole e minuscole. L&#39;unico parametro obbligatorio passato a `s7ComponentEvent` è l&#39;ultimo, `eventData`. Dove `s7track()` è definito in s_code.jsp incluso in precedenza. E `s7track` elabora tutto il tracciamento per ogni evento. (Puoi personalizzare ulteriormente i dati trasmessi ad Adobe Analytics in quest’area.)
