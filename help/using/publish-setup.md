...
Per aprire le pagine di Impostazione pubblicazione, vai a **[!UICONTROL Configurazione]** > **[!UICONTROL Impostazione applicazione]** > **[!UICONTROL Impostazione pubblicazione]**.
...
Se `locale=` non è specificato, `attribute::DefaultLocale` viene passato ai motori di traduzione. La convalida dell’input è limitata `locale=` valore. Vuoto `locale=` sono consentiti. Perché `locale=` ha portata globale, `attribute::DefaultLocale` viene fornito dal catalogo principale per l’intera richiesta.

Alcuni dei vantaggi dell’utilizzo di `locale=` e `attribute::DefaultLocale` includi quanto segue:
...
Con la localizzazione, un URL del server immagini aggiunge `locale=` al percorso, come indicato di seguito:

`https://server/is/image/company/image?locale=de_DE`

Quando il server immagini riceve la chiamata http, `locale=` Il parametro viene analizzato attraverso `localeMap` campo trovato in **[!UICONTROL Configurazione]** > **[!UICONTROL Impostazione applicazione]** > **[!UICONTROL Impostazione pubblicazione]** > **[!UICONTROL Server immagini]** > **[!UICONTROL Supporto per la localizzazione]** gruppo.
...
Puoi anche usare un suffisso locale dedicato, ad esempio U, solo per le lingue sconosciute e forzare l’immagine predefinita in caso contrario `_U` esiste, come nei seguenti casi:
...
Utilizzando il primo esempio come base, le immagini per tutte le lingue potrebbero avere i suffissi `_1`, `_2`, o `_3`. Le immagini specifiche delle lingue francesi potrebbero avere i suffissi `_22` o `_23` suffisso. E le immagini specifiche delle lingue tedesche potrebbero avere i suffissi `_470` o `_480`.
