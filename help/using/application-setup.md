...
* **[!UICONTROL Nome server pubblicato]**: questo server è il server Live CDN (Content Delivery Network) utilizzato in tutte le chiamate URL generate dal sistema e specifiche per il tuo account. Modificare il nome del server solo se un rappresentante del supporto tecnico di Adobe Dynamic Media Classic lo richiede.

* **[!UICONTROL Nome server di origine]**: questo server viene utilizzato solo per i test di controllo qualità. Modificare il nome del server solo se un rappresentante del supporto tecnico di Adobe Dynamic Media Classic lo richiede.
...
* **[!UICONTROL Modello mappatura immagine predefinito]**: definisce il JavaScript predefinito per il modello HREF nella mappatura immagine. Per eseguire un codice personalizzato ogni volta che si seleziona una mappa immagine, impostarla qui.

### Altre impostazioni (su applicazione)

* **[!UICONTROL Avvisi di eliminazione del cestino]**: Assets nella cartella Cestino viene eliminato automaticamente entro sette giorni. Seleziona &quot;Invia e-mail prima che gli elementi nella cartella Cestino vengano eliminati automaticamente&quot;. Questa azione invia notifiche agli amministratori della società. Ciò si verifica quando le risorse nella cartella Cestino sono pianificate per l’eliminazione definitiva tra quattro giorni. Vedi [Gestione cartella cestino](/help/using/trash-folder.md).
...
Le schermate Aggiungi predefinito e Modifica predefinito offrono le seguenti opzioni per la creazione e la modifica di predefiniti immagine:
...
* **[!UICONTROL Soglia]**: per ignorare un intervallo di contrasto quando si applica il filtro Maschera di contrasto, utilizzare questa impostazione. In altre parole, può aiutare a stabilire quanto devono differire i pixel resi più nitidi dall’area circostante prima che vengano considerati pixel del bordo e più nitidi. Per evitare di introdurre disturbi, provare con valori compresi tra `.02` e `0.2`. Il valore predefinito 6 applica la nitidezza a tutti i pixel dell&#39;immagine.
...
Per impostazione predefinita, sono attivati solo i predefiniti di codifica &quot;Video adattivo&quot; (abilitati o &quot;attivati&quot;). Puoi disattivarli, se necessario. I predefiniti per video adattivi non attivati non vengono visualizzati come opzione selezionabile nella sezione eVideo della finestra di dialogo Opzioni processo di caricamento.
...
1. Per rimuovere un predefinito dall&#39;elenco Opzioni eVideo della finestra di dialogo Opzioni processo di caricamento, deselezionate la casella di controllo accanto al nome di un predefinito nella pagina Predefiniti video adattivi.
...
>[!NOTE]
>
>Ad eccezione di &quot;Video adattivo&quot;, che Adobe Dynamic Media Classic abilita per impostazione predefinita, gli amministratori di Adobe Dynamic Media Classic determinano quali predefiniti di codifica video a codifica singola sono visibili nella finestra di dialogo Opzioni processo di caricamento. Gli amministratori di Adobe Dynamic Media Classic determinano quali predefiniti di codifica video sono visibili nella finestra di dialogo Opzioni processo di caricamento.
...
Un *predefinito per visualizzatori* è un gruppo di impostazioni con cui viene definito in che modo le risorse multimediali verranno visualizzate sullo schermo del computer e sui dispositivi mobili dell’utente. In qualità di amministratore, potete creare dei predefiniti per visualizzatori. Sono disponibili impostazioni per un’ampia gamma di opzioni di configurazione dei visualizzatori. Ad esempio, è possibile modificare le dimensioni di visualizzazione, il comportamento di zoom e l&#39;aspetto del visualizzatore.
...
Combinando i seguenti elementi in un singolo lettore:
...
* **[!UICONTROL Visualizzatore zoom: a comparsa]**: visualizza una seconda immagine dell&#39;area ingrandita accanto all&#39;immagine originale. Non sono presenti controlli; gli utenti spostano la selezione sull&#39;area che desiderano visualizzare.
...
* I dispositivi BlackBerry® che supportano questo formato video sono disponibili nei seguenti percorsi: [Formati video supportati in BlackBerry®](https://www.blackberry.com/en/secure-communications/support/developers)
...
|--- |--- |--- |--- |--- |--- |--- |--- |--- |
| Universal_HTML5_Video(Include il supporto per i sottotitoli.) Consulta [Best practice: utilizzo del visualizzatore video Universal HTML5].(best-practice-utilizzo-html5-video.md#best_practice_utilizzo_html5_video_viewer) | HTML5 | X | X | X | X | X | X | X |
...
Oltre ad aggiungere i predefiniti visualizzatore utilizzando **[!UICONTROL Aggiungi]** nell&#39;interfaccia utente, puoi anche utilizzare **[!UICONTROL Esporta]** per aggiungere un predefinito visualizzatore. È possibile esportare un predefinito visualizzatore HTML5 esistente e utilizzarlo come base per il nuovo predefinito.
...
Per creare un predefinito visualizzatore HTML5, esportate un predefinito visualizzatore HTML5 esistente da utilizzare come base. Questa opzione di esportazione è utile perché non è necessario creare il visualizzatore dall&#39;inizio. È invece possibile esportare un predefinito simile a quello desiderato e utilizzarlo come base per apportare modifiche alla struttura.

Tutti i file CSS predefiniti e standard del predefinito visualizzatore in Adobe Dynamic Media Classic utilizzano percorsi relativi di gestione delle immagini che puntano alle risorse in `Scene7SharedAssets`. Ad esempio, il percorso relativo di una risorsa immagine in un file CSS del predefinito per visualizzatori in

`Scene7SharedAsset`: `.s7videoviewer .s7fullscreenbutton[state][selected] { background-image: url(/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha); }`

Tuttavia, se ospiti i file CSS del visualizzatore sul tuo sito, devi risolvere questi percorsi immagine relativi utilizzando un percorso esplicito al server immagini nel tuo ambiente. Ad esempio, se hai aggiornato il percorso relativo precedente a un percorso esplicito, sarà simile al seguente, dove `https://s7d1.scene7.com` è il percorso diretto del server immagini: `https://s7d1.scene7.com/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha`
...
1. Effettuate una delle seguenti operazioni:
   ...
   * Per visualizzare un’anteprima di una visualizzazione, selezionatela. I campi di visualizzazione vengono visualizzati nel pannello Anteprima.
...
I predefiniti di metadati consentono agli amministratori di controllare e regolare i metadati assegnati alle risorse. Nella Vista dettagli, un utente può immettere i metadati di una risorsa nei campi forniti a tale scopo. Ad esempio, un utente può inserire un nome proprietario, una descrizione del copyright e un indirizzo. Per fare in modo che gli utenti immettano con precisione queste informazioni, puoi creare predefiniti di metadati. Quando si sceglie un predefinito di metadati nella vista Dettaglio, i campi di metadati vengono compilati con valori predefiniti. Ad esempio, il nome proprietario, la descrizione del copyright e l’indirizzo vengono inseriti automaticamente.
...
1. Espandi le intestazioni da includere nel predefinito. Immettete i valori nei diversi campi da includere nel predefinito.
1. Seleziona **[!UICONTROL Salva]**.
...
Un amministratore Media Portal o un amministratore aziendale può creare campi di metadati personalizzati e definiti dall’utente. I campi personalizzati consentono di organizzare le risorse in Adobe Dynamic Media Classic. Se necessario, puoi contrassegnare i campi come Attivi. Quando l&#39;opzione è attivata, i nomi dei campi di metadati personalizzati vengono visualizzati nel pannello Metadati visualizzazione dettagli. Nei campi di metadati definiti dall’utente gli utenti possono immettere informazioni per descrivere le risorse. Gli utenti possono inoltre utilizzare tali campi di metadati come criteri di ricerca.
...
1. Nella finestra di dialogo Campo personalizzato, impostate le opzioni desiderate.
   ...
   * **[!UICONTROL Valore predefinito]**: facoltativo. Immetti il valore che gli utenti immettono più spesso nel campo. Il valore immesso diventa il valore predefinito per il nuovo campo creato.

   * **[!UICONTROL Si Applica A]**: Facoltativo. Seleziona un tipo di risorsa se desideri che il campo metadati sia applicabile solo a un tipo di risorsa specifico.
...
Solo un amministratore di Media Portal o un amministratore aziendale può gestire i campi definiti dall&#39;utente.
...
Gli amministratori aziendali definiscono innanzitutto le convenzioni di denominazione per le risorse che desiderano raggruppare in un set. Per fare riferimento a queste immagini, crea un predefinito per set di batch. Ogni predefinito è un set di istruzioni indipendente con un nome univoco. Definisce come creare il set utilizzando immagini che corrispondono alle convenzioni di denominazione definite nella ricetta predefinita.
...
1. Selezionate **[!UICONTROL Visualizza modulo]** o **[!UICONTROL Visualizza codice]** per specificare la modalità di visualizzazione e di inserimento delle informazioni relative a ciascun elemento.
   ...
   È possibile selezionare la casella di controllo **[!UICONTROL Visualizza codice]** per visualizzare la creazione del valore dell&#39;espressione regolare insieme alle selezioni del modulo. È possibile immettere o modificare questi valori per definire gli elementi della convenzione di denominazione, se per qualsiasi motivo la visualizzazione Maschera vi limita. Se non è possibile analizzare i valori nella visualizzazione Maschera, i campi modulo diventano inattivi.
...
Per la definizione sono disponibili due elementi, Corrispondenza e Nome base. Questi campi sono tutti gli elementi definiti per una convenzione di denominazione. Contribuiscono a identificare la parte della convenzione utilizzata per denominare il set. Questo set li contiene. La convenzione di denominazione individuale di un’azienda utilizza una o più righe di definizione per ciascuno di questi elementi. È possibile utilizzare tutte le righe per la definizione univoca e raggrupparle in elementi distinti, ad esempio immagine, colore e visualizzazione.
...
Utilizzare Predefinito set di batch per creare, modificare e gestire i predefiniti set di batch. Per coprire tutti i processi di acquisizione risorse richiesti, crea tutti i predefiniti necessari. Esistono due forme di definizioni dei predefiniti per set di batch: una per una convenzione di denominazione predefinita che hai impostato e una per le convenzioni di denominazione personalizzate che crei immediatamente.

È possibile utilizzare il metodo campo modulo per definire un predefinito per set di batch o il metodo di codice, che consente di utilizzare espressioni regolari. Come in **[!UICONTROL Denominazione predefinita]**, è possibile selezionare **[!UICONTROL Visualizzazione codice]** contemporaneamente alla definizione nella Visualizzazione modulo e utilizzare espressioni regolari per creare le definizioni. In alternativa, potete deselezionare una delle due visualizzazioni e usare solo l’altra.
...
È possibile utilizzare il tipo di set di batch **Set 360 gradi a più assi** per creare una &quot;ricetta&quot; che automatizza la generazione di set 360 gradi 2D. Il raggruppamento di immagini utilizza le espressioni regolari Row e Column per allineare le risorse immagine nella matrice multidimensionale.
...
Con queste informazioni, la composizione del tipo di set di batch viene creata come segue:
...
Quando il set 360 gradi viene caricato e pubblicato, attiva il nome della definizione del set 360 gradi 2D elencata in **[!UICONTROL Predefiniti set di batch]** nella finestra di dialogo Opzioni processo di caricamento.
...
1. Utilizzate gli attributi **[!UICONTROL Corrispondenza]** e, facoltativamente, **[!UICONTROL Nome base]** per definire un’espressione regolare per la denominazione delle risorse di immagine che compongono il gruppo.
   ...
   Ad esempio, l’espressione regolare Match letterale si presenta come segue:
...
1. Espandete **[!UICONTROL Posizione colonna riga]** e definite il formato del nome per la posizione della risorsa di immagine nell’array set 360 gradi 2D.
   ...
   L&#39;espressione regolare della riga, ad esempio, è simile alla seguente:
   ...
   L’espressione regolare della colonna è simile alla seguente:
